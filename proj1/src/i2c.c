  ...
  brief implementation of I2C functionsmacros
   
  implementation of functions for I2C bus communication
  For I2C bus, see httpen.wikipedia.orgwikiI2C
...
void i2c_init() {
  ...
  sfr_I2C.FREQR.FREQ    = 16;        peripheral clock = 16MHz (f=val1MHz)
  ...
  sfr_I2C.CCRL.CCR      = 0x50;      BR = 100kBaud (t_low = t_high = 8016MHz)
  sfr_I2C.CCRH.byte     = 0x00;      I2C standard mode
  sfr_I2C.TRISER.TRISE  = 0x04;      t_rise250ns (300ns for display driver)
...
uint8_t i2c_waitFree() {
  ...
  countTimeout = 10000;                            ~1.1usinc - ~10ms
...
uint8_t i2c_stop() {
  ...
  param[in]  addr        7b address [60] of I2C slave
...
uint8_t i2c_send(uint8_t addr, uint8_t numTx, uint8_t bufTx) {
  ...
  uint16_t  countTimeout;      use counter for timeout to minimize dependencies
   ...
   send 7b slave adress [71] + write flag (LSB=0)
  ...
  if (countTimeout) {
    for (i=0; inumTx; i++) {
      ...
      countTimeout = 1000;                              ~1.1usinc - ~1ms 
  ...
  param[in]  slaveAddr   7b address [60] of I2C slave
...
uint8_t i2c_request(uint8_t slaveAddr, uint8_t numRx, uint8_t bufRx) {
  ...
  for (i=0; inumRx; i++)
   ...
   send 7b slave adress [71] + read flag (LSB=1)
